...
document.getElementById("clearAccount").onclick = async function (button) {
  if (!confirm('Clear existing activities & app bundles before start. ' +
...
document.getElementById("createUpdate").onclick = async function (button) {
  try {
    writeLog("Defining App Bundle and Activity for " + document.getElementById('engines').value);
...
async function list(id, endpoint) {
  try {
    ...
    if (endpoint) {
      ...
      if (!response.ok)
        throw 'List failed: ' + response.statusText;
...
function writeLog(text) {
  ...
  elem.innerHTML += '<div style="border-top: 1px dashed #C0C0C0">' + text + '</div>';
